...
def build_output_line():
    ...
    # csv_row uses "" for missing DXY fields, since that's what the CSV
    # writer needs for a clean blank cell.
    csv_row = {
    ...
    # today_row is the typed version used everywhere else (scoring, gap
    # detection): missing DXY fields are None, not "", so downstream
    # comparisons (`if x is not None`, `x > threshold`) behave correctly
    # instead of trying to compare a string against a float.
    today_row = {
        **csv_row,
        "dxy_price": dxy["price"] if dxy else None,
        "dxy_change_pct_24h": dxy["changePercent24h"] if dxy else None,
    }
 
    history = load_history()
 
    # Avoid double-appending if the workflow is run more than once same day
    if history and history[-1]["date"] == csv_row["date"]:
        rows_with_today = history
    else:
        append_history_row(csv_row)
        rows_with_today = history + [today_row]
